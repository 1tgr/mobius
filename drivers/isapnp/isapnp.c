/* $Id: isapnp.c,v 1.1 2002/12/21 09:48:49 pavlovskii Exp $ */
...
static int pnp_read_one_resource(isapnp_t *pnp, isacard_t *card)
{
    ...
    case ISAPNP_SRIN_LDEVICE_ID:
        ...
        swprintf(card->name, L"isa:%c%c%c%04x:%d",
            ((i & 0x001f) >> 0) + 'A' - 1,
            ((i & 0x03e0) >> 5) + 'A' - 1,
            ((i & 0x7c00) >> 10) + 'A' - 1,
            (buffer[3] << 8) | buffer[2],
            card->cfg.location.isa.number);
        card->cfg.vendor_id = i;
        card->cfg.device_id = (buffer[3] << 8) | buffer[2];
...
bool PnpRequest(device_t* dev, request_t* req)
{
    switch (req->code)
    {
    case DEV_REMOVE:
...
void PnpInitCard(isapnp_t *pnp, isacard_t *card)
{
    ...
    card->cfg.location.isa.number = pnp->boards_found;
    card->cfg.parent = &pnp->dev;
}
...
void PnpAddCard(isapnp_t *pnp, isacard_t *card)
{
    ...
    swprintf(key, L"ISA/%c%c%c%04x", 
        ((card->cfg.vendor_id & 0x001f) >> 0) + 'A' - 1,
        ((card->cfg.vendor_id & 0x03e0) >> 5) + 'A' - 1,
        ((card->cfg.vendor_id & 0x7c00) >> 10) + 'A' - 1,
	card->cfg.device_id);
    wprintf(L"isapnp: %s\n", key);
