...
#define malloc_getpagesize	(4 * 4096)
...
#define amALLINFo   public_amALLINFo
#define amALLOPt    public_amALLOPt
#define amTRIm      public_amTRIm
//#define amSTATs     public_amSTATs
#define amUSABLe    public_amUSABLe
...
#define public_amTRIm    dlmalloc_atrim
//#define public_amSTATs   dlmalloc_astats
#define public_amUSABLe  dlmalloc_usable_size
...
#define public_amTRIm    malloc_atrim
//#define public_amSTATs   malloc_astats
#define public_amUSABLe  malloc_usable_size
...
*/
#if __STD_C
int      public_amTRIm(mstate, size_t);
#else
int      public_amTRIm();
#endif
...
#if __STD_C
static Void_t*  amALLOc(mstate, size_t);
static void     afREe(mstate, Void_t*);
static Void_t*  arEALLOc(mstate, Void_t*, size_t);
static Void_t*  amEMALIGn(mstate, size_t, size_t);
static Void_t*  avALLOc(mstate, size_t);
static Void_t*  apVALLOc(mstate, size_t);
static Void_t*  acALLOc(mstate, size_t, size_t);
static Void_t** iaCALLOc(mstate, size_t, size_t, Void_t**);
static Void_t** iaCOMALLOc(mstate, size_t, size_t*, Void_t**);
static void     acFREe(mstate, Void_t*);
static int      amTRIm(mstate, size_t);
static size_t   amUSABLe(mstate, Void_t*);
//static void     amSTATs(mstate);
static int      amALLOPt(mstate, int, int);
static struct mallinfo amALLINFo(mstate);
#else
static Void_t*  amALLOc();
static void     afREe();
static Void_t*  arEALLOc();
static Void_t*  amEMALIGn();
static Void_t*  avALLOc();
static Void_t*  apVALLOc();
static Void_t*  acALLOc();
static Void_t** iaCALLOc();
static Void_t** iaCOMALLOc();
static void     acFREe();
static int      amTRIm();
static size_t   amUSABLe();
//static void     amSTATs();
static int      amALLOPt();
static struct mallinfo amALLINFo(mstate);
#endif
...
#else
int amTRIm(av, pad) mstate av; size_t pad;
#endif
{
  /*mstate av = get_malloc_state();*/
  /* Ensure initialization/consolidation */
...
#if __STD_C
size_t amUSABLe(mstate av, Void_t* mem)
#else
size_t amUSABLe(mem) Void_t* mem;
#endif
{
  mchunkptr p;
  if (mem != 0) {
...
#else
int amALLOPt(av, param_number, value) mstate av; int param_number; int value;
#endif
{
  /*mstate av = get_malloc_state();*/
  /* Ensure initialization/consolidation */
...
#define MALLOC_PREACTION	__malloc_lock(1, av->lock)
#define MALLOC_POSTACTION	__malloc_lock(0, av->lock)
...
Void_t* public_avALLOc(mstate av, size_t bytes) {
  Void_t* m;
  if (MALLOC_PREACTION != 0) {
    return 0;
  }
...
Void_t* public_apVALLOc(mstate av, size_t bytes) {
  Void_t* m;
  if (MALLOC_PREACTION != 0) {
    return 0;
  }
...
void public_acFREe(mstate av, Void_t* m) {
  if (MALLOC_PREACTION != 0) {
    return;
  }
  acFREe(av, m);
...
int public_amTRIm(mstate av, size_t s) {
  int result;
  if (MALLOC_PREACTION != 0) {
    return 0;
  }
...
size_t public_amUSABLe(mstate av, Void_t* m) {
  size_t result;
  if (MALLOC_PREACTION != 0) {
    return 0;
  }
