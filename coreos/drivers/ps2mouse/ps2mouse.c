...
static void ps2DeleteDevice(device_t *device)
{
	/* xxx - shut down mouse */
	/*DevRegisterIrq(12, device, false);*/
	free(device->cookie);
}
...
static status_t ps2Request(device_t *device, request_t* req)
{
	...
	case DEV_READ:
		...
		io = DevQueueRequest(device, &req_dev->header, sizeof(*req_dev),
...
void ps2AddDevice(driver_t* drv, const wchar_t* name, dev_config_t* cfg)
{
