...
void KbdStartIo(keyboard_t* keyb)
{
    ...
    while (keyb->device->io_first && keyb->read != keyb->write)
    {
        key = kbdRead(keyb);
        for (io = keyb->device->io_first; io; io = ionext)
        {
...
static void KbdDeleteDevice(device_t *device)
{
	/* DevRegisterIrq(1, NULL); */
	free(device->cookie);
}


static status_t KbdRequest(device_t* device, request_t* req)
{
    keyboard_t* keyb = device->cookie;
    ...
    case DEV_READ:
        if (req_dev->params.dev_read.length % sizeof(uint32_t))
            return req_dev->header.code = EINVALID;
    ...
    case DEV_IOCTL:
        if (!MemVerifyBuffer(req_dev->params.dev_ioctl.params, 
            ...
            return req_dev->header.result = EBUFFER;
...
static bool KbdIsr(device_t *device, uint8_t irq)
{
    keyboard_t* keyb = device->cookie;
    uint32_t key;
    uint8_t scancode;
...
void KbdAddDevice(driver_t* drv, const wchar_t *name, dev_config_t *cfg)
{
    keyboard_t* keyb;
    uint16_t port, ctrl;
	...
	kbdHwWrite(keyb, keyb->ctrl, KCTRL_READ_CMD_BYTE);
	wprintf(L"keyboard: command byte = %02x, new = %02x\n", 
		kbdHwRead(keyb), KCTRL_TRANSLATE_XT | KCTRL_IRQ1 | KCTRL_IRQ12);

    kbdHwWrite(keyb, keyb->ctrl, KCTRL_WRITE_CMD_BYTE);
    kbdHwWrite(keyb, keyb->port, KCTRL_TRANSLATE_XT | KCTRL_IRQ1 | KCTRL_IRQ12);
    ...
    keymap = ProGetString(cfg->profile_key, L"DefaultMap", 
        SYS_BOOT L"/british.kbd");
    file = FsOpen(NULL, keymap, FILE_READ);
    if (file == NULL)
    {
    ...
    {
        FsRead(file, keys, 0, sizeof(keys), NULL);
        HndClose(&file->hdr);
    }

	if (cfg->device_class == 0)
		cfg->device_class = 0x0900;

    keyb->device = DevAddDevice(drv, &keyboard_vtbl, 0, name, cfg, keyb);
	DevRegisterIrq(1, keyb->device);
}
