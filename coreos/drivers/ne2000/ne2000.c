/* $Id: ne2000.c,v 1.1.1.1 2002/12/21 09:48:59 pavlovskii Exp $ */
...
static void NeDeleteDevice(device_t *device)
{
	free(device->cookie);
}


static status_t NeRequest(device_t *device, request_t* req)
{
	return ENOTIMPL;
}


static bool NeIsr(device_t *device, uint8_t irq)
{
    ne2000_t *ne;
    uint8_t isr;
...
void NeAddDevice(driver_t *drv, const wchar_t *name, dev_config_t *cfg)
{
	ne2000_t* ne;
	...
	ne->device = DevAddDevice(drv, &ne_vtbl, 0, name, cfg, ne);
    DevRegisterIrq(12, ne->device);
