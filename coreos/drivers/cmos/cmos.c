/* $Id: cmos.c,v 1.1.1.1 2002/12/21 09:48:40 pavlovskii Exp $ */
...
status_t rtcRequest(device_t* dev, request_t* req)
{
	uint64_t* qw;
	...
	case DEV_READ:
		...
		*qw = sys_time();
		MemUnmapPageArray(req_dev->params.dev_read.pages);
		return 0;
	}
...
void cmosAddDevice(driver_t* drv, const wchar_t* name, dev_config_t* cfg)
{
	TRACE2("CMOS %s installed; time = %u\n", name, sys_time());
	DevAddDevice(drv, &cmos_vtbl, 0, name, cfg, NULL);
}
