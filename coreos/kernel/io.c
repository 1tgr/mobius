/* $Id: io.c,v 1.1.1.1 2002/12/21 09:49:12 pavlovskii Exp $ */
...
status_t IoRequest(const io_callback_t *cb, device_t *dev, request_t *req)
{
    status_t ret;
...
status_t IoRequestSync(device_t *dev, request_t *req)
{
    io_callback_t cb;
    syncrequest_t sync;
	status_t ret;
...
size_t IoReadPhysicalSync(device_t *dev, uint64_t ofs, page_array_t *array, size_t size)
{
    ...
    if (IoRequestSync(dev, &req.header) == 0)
