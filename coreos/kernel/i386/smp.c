/* $Id: smp.c,v 1.1.1.1 2002/12/21 09:49:35 pavlovskii Exp $ */
...
void i386MpProbe(void)
{
    ...
    if (kernel_startup.num_cpus > 1)
    {
        ...
        MemMapRange((addr_t) apic, apic_phys, (addr_t) apic + PAGE_SIZE, 
            PRIV_PRES | PRIV_KERN | PRIV_RD | PRIV_WR);
        MemMapRange((addr_t) ioapic, ioapic_phys, (addr_t) ioapic + PAGE_SIZE, 
            PRIV_PRES | PRIV_KERN | PRIV_RD | PRIV_WR);
    }
}
