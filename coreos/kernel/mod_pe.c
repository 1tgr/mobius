 ...
 * 
 * *****************  Version 2  *****************
 * User: Tim          Date: 13/03/04   Time: 22:21
 * Updated in $/coreos/kernel
 * Added call to VmmFree in PeUnload
 * Tidied up
 * Added history block
 */
...
module_t* PeLoad(process_t* proc, const wchar_t* file, uint32_t base)
{
    ...
    size_t size;
    const wchar_t *temp;
    wchar_t *full_file, *search_path, *ch;
    module_info_t *info;
    ...
    full_file = malloc(sizeof(wchar_t) * MAX_PATH);
    if (file[0] == '/')
        wcsncpy(full_file, file, MAX_PATH);
    ...
    fd = FsOpen(NULL, full_file, FILE_READ);
    if (!fd)
    {
        wprintf(L"PeLoad: failed to open %s\n", full_file);
        KeAtomicDec(&nesting);
        free(full_file);
        return NULL;
    }

    if (!FsRead(fd, &dos, 0, sizeof(dos), &size) ||
        size < sizeof(dos))
    {
    ...
    if (!FsRead(fd, &pe, dos.e_lfanew, sizeof(pe), &size) ||
        size < sizeof(pe))
    {
    ...
    if (!mod->imported)
        mod->imported = PeDoImports(proc, mod, pe.OptionalHeader.DataDirectory);

    if (!mod->imported)
        wprintf(L"PeLoad(%s): imports failed\n", file);
    ...
    if (mod->base >= 0x80000000)
    {
        ...
        for (virt = mod->base; 
            virt < mod->base + mod->length; 
            virt += PAGE_SIZE)
            a = *(volatile const uint8_t*) virt;
    }
...
addr_t PeGetExport(module_t* mod, const char* name, uint16_t hint)
{
    ...
    {
        wprintf(L"%s: DLL has no exported name table (%S), VA = %p\n",
            mod->name, name, exp);
        __asm__("int3");
        return NULL;
    }
...
void PeUnload(process_t* proc, module_t* mod)
{
    ...
    if (mod->refs == 0)
    {
        LIST_REMOVE(proc->mod, mod);
        HndClose(&mod->file->hdr);
