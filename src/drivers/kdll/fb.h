...
struct fb_fix_screeninfo {
        wchar_t id[16];                    /* identification string eg "TT Builtin" */
        ...
        dword accel;                    /* Type of acceleration available */
...
struct fb_var_screeninfo {
        ...
        dword accel_flags;              /* acceleration flags (hints)   */
        ...
        dword reserved[6];              /* Reserved for future compatibility */
};

struct fb_cmap {
        dword start;                    /* First entry  */
        dword len;                      /* Number of entries */
        word *red;                     /* Red values   */
        word *green;
        word *blue;
        word *transp;                  /* transparency, can be NULL */
};
...
struct fb_monspecs {
        dword hfmin;                    /* hfreq lower limit (Hz) */
        dword hfmax;                    /* hfreq upper limit (Hz) */
        word vfmin;                    /* vfreq lower limit (Hz) */
        word vfmax;                    /* vfreq upper limit (Hz) */
