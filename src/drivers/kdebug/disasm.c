...
static wchar_t *whichcond(int condval) 
{
    static int conds[] = {
	     ...
	     int autosync, unsigned long prefer)
{
	...
	if (*data == 0xF3 || *data == 0xF2)
	    ...
	    switch (*data++) {
	      case 0x2E: segover = L"cs"; break;
	      case 0x36: segover = L"ss"; break;
	      case 0x3E: segover = L"ds"; break;
	      case 0x26: segover = L"es"; break;
	      case 0x64: segover = L"fs"; break;
	      case 0x65: segover = L"gs"; break;
	    }
	} else if (*data == 0x66)
    ...
    if (lock)
	slen += swprintf(output+slen, L"lock ");
    for (i = 0; i < ins.nprefix; i++)
	switch (ins.prefixes[i]) {
	case P_REP:   slen += swprintf(output+slen, L"rep "); break;
	  case P_REPE:  slen += swprintf(output+slen, L"repe "); break;
	  case P_REPNE: slen += swprintf(output+slen, L"repne "); break;
	  case P_A16:   slen += swprintf(output+slen, L"a16 "); break;
	  case P_A32:   slen += swprintf(output+slen, L"a32 "); break;
	  case P_O16:   slen += swprintf(output+slen, L"o16 "); break;
	  case P_O32:   slen += swprintf(output+slen, L"o32 "); break;
	}

    for (i = 0; i < elements(ico); i++)
	if ((*p)->opcode == ico[i]) {
	    slen += swprintf(output+slen, L"%s%s", icn[i],
			    whichcond(ins.condition));
	    break;
	}
    if (i >= elements(ico))
	slen += swprintf(output+slen, L"%s", insn_names[(*p)->opcode]);
    colon = FALSE;
	    ...
	    (ins.oprs[i].segment & SEG_RMREG)) 
	{
	    ins.oprs[i].basereg = whichreg ((*p)->opd[i],
					    ins.oprs[i].basereg);
	    if ( (*p)->opd[i] & TO )
		slen += swprintf(output+slen, L"to ");
	    slen += swprintf(output+slen, L"%s",
			    reg_names[ins.oprs[i].basereg-EXPR_REG_START]);
	} else if (!(UNITY & ~(*p)->opd[i])) {
	    output[slen++] = '1';
	} else if ( (*p)->opd[i] & IMMEDIATE ) {
	    if ( (*p)->opd[i] & BITS8 ) {
		slen += swprintf(output+slen, L"byte ");
		...
		slen += swprintf(output+slen, L"word ");
	    } else if ( (*p)->opd[i] & BITS32 ) {
		slen += swprintf(output+slen, L"dword ");
	    } else if ( (*p)->opd[i] & NEAR ) {
		slen += swprintf(output+slen, L"near ");
	    } else if ( (*p)->opd[i] & SHORT ) {
		slen += swprintf(output+slen, L"short ");
	    }
	    //slen += sprintf(output+slen, "0x%lx", ins.oprs[i].offset);
		slen += add_label(*p, &ins, output+slen, i);
	} else if ( !(MEM_OFFS & ~(*p)->opd[i]) ) {
		wchar_t buf[50];
		add_label(*p, &ins, buf, i);
	    slen += swprintf(output+slen, L"[%s%s%s%s]",
			    (segover ? segover : L""),
			    (segover ? L":" : L""),
			    (ins.oprs[i].addr_size == 32 ? L"dword " :
			     ins.oprs[i].addr_size == 16 ? L"word " : L""),
			    buf);
	    segover = NULL;
		...
		slen += swprintf(output+slen, L"dword ");
	    if ( (*p)->opd[i] & BITS64 )
		slen += swprintf(output+slen, L"qword ");
	    if ( (*p)->opd[i] & BITS80 )
		slen += swprintf(output+slen, L"tword ");
	    if ( (*p)->opd[i] & FAR )
		slen += swprintf(output+slen, L"far ");
	    if ( (*p)->opd[i] & NEAR )
		slen += swprintf(output+slen, L"near ");
	    output[slen++] = '[';
		...
		slen += swprintf(output+slen, L"%s",
				(ins.oprs[i].addr_size == 32 ? L"dword " :
				 ins.oprs[i].addr_size == 16 ? L"word " : L""));
	    if (segover) {
		slen += swprintf(output+slen, L"%s:", segover);
		segover = NULL;
	    }
		...
		if (ins.oprs[i].scale > 1)
		    slen += swprintf(output+slen, L"*%d", ins.oprs[i].scale);
		started = TRUE;
	    }
		...
		slen += swprintf(output+slen, L"%c0x%lx", sign,
				ins.oprs[i].offset);
	    ...
	    slen += swprintf(output+slen, L"<operand%d>", i);
	}
    }
    output[slen] = '\0';
	...
	while (count--)
	    p[count+3] = p[count];
	wcsncpy(output, segover, 2);
	output[2] = ' ';
    }
    return length;
}

long eatbyte (unsigned char *data, wchar_t *output) 
{
    swprintf(output, L"db 0x%02X", *data);
    return 1;
}
