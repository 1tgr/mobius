 ...
 */
int i386_registers[NUMREGBYTES/4];
...
asm("_return_to_prog:");
asm("				movw _i386_registers+44, %ss");
asm("				movl _i386_registers+16, %esp");
asm("				movl _i386_registers+4, %ecx");
asm("				movl _i386_registers+8, %edx");
asm("				movl _i386_registers+12, %ebx");
asm("				movl _i386_registers+20, %ebp");
asm("				movl _i386_registers+24, %esi");
asm("				movl _i386_registers+28, %edi");
asm("				movw _i386_registers+48, %ds");
asm("				movw _i386_registers+52, %es");
asm("				movw _i386_registers+56, %fs");
asm("				movw _i386_registers+60, %gs");
asm("				movl _i386_registers+36, %eax");
asm("				pushl %eax");	/* saved eflags */
asm("				movl _i386_registers+40, %eax");
asm("				pushl %eax");	/* saved cs */
asm("				movl _i386_registers+32, %eax");
asm("				pushl %eax");	/* saved eip */
asm("				movl _i386_registers, %eax");
...
#define SAVE_REGISTERS1() \
	asm ("movl %eax, _i386_registers");							\
	asm ("movl %ecx, _i386_registers+4"); 					 \
	asm ("movl %edx, _i386_registers+8"); 					 \
	asm ("movl %ebx, _i386_registers+12");						 \
	asm ("movl %ebp, _i386_registers+20");						 \
	asm ("movl %esi, _i386_registers+24");						 \
	asm ("movl %edi, _i386_registers+28");						 \
	asm ("movw $0, %ax"); 							 \
	asm ("movw %ds, _i386_registers+48"); 					 \
	asm ("movw %ax, _i386_registers+50"); 					 \
	asm ("movw %es, _i386_registers+52"); 					 \
	asm ("movw %ax, _i386_registers+54"); 					 \
	asm ("movw %fs, _i386_registers+56"); 					 \
	asm ("movw %ax, _i386_registers+58"); 					 \
	asm ("movw %gs, _i386_registers+60"); 					 \
	asm ("movw %ax, _i386_registers+62");
#define SAVE_ERRCODE() \
	asm ("popl %ebx");						\
	asm ("movl %ebx, _gdb_i386errcode");
#define SAVE_REGISTERS2() \
	asm ("popl %ebx"); /* old eip */						 \
	asm ("movl %ebx, _i386_registers+32");						 \
	asm ("popl %ebx");	 /* old cs */						 \
	asm ("movl %ebx, _i386_registers+40");						 \
	asm ("movw %ax, _i386_registers+42"); 						\
	asm ("popl %ebx");	 /* old eflags */					 \
	asm ("movl %ebx, _i386_registers+36");						 \
	/* Now that we've done the pops, we can save the stack pointer.");	*/	 \
	asm ("movw %ss, _i386_registers+44"); 					 \
	asm ("movw %ax, _i386_registers+46"); 						 \
	asm ("movl %esp, _i386_registers+16");
...
char * buffer;
{
	...
	wprintf(L"bad checksum.	My count = 0x%x, sent=0x%x. buf=%S\n",
...
void handle_exception(int exceptionVector)
{
	...
	switch (remcomInBuffer[0])
	{
		...
		mem2hex((char*) i386_registers, remcomOutBuffer, NUMREGBYTES, 0);
		break;
		case 'G' : /* set the value of the CPU registers - return OK */
		hex2mem(&remcomInBuffer[1], (char*) i386_registers, NUMREGBYTES, 0);
	 ...
	 ptr = &remcomInBuffer[1];
	 if (hexToInt(&ptr,&addr))
		 i386_registers[ PC ] = addr;

		newPC = i386_registers[ PC];

		/* clear the trace bit */
		i386_registers[ PS ] &= 0xfffffeff;

		/* set the trace bit if we're stepping */
		if (remcomInBuffer[0] == 's') i386_registers[ PS ] |= 0x100;
		...
		/* Don't really think we need this, except maybe for protection
		 exceptions.	*/
			...
			newPC = i386_registers[ PC ];		/* pc may have changed	*/
#endif /* 0 */
