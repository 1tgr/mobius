/* $Id: port.old.c,v 1.2 2002/04/20 12:30:03 pavlovskii Exp $ */
...
bool PortDoPortRequest(device_t *dev, uint32_t code, params_port_t *params, status_t *result)
{
    port_t *port, *remote;
    ...
    {
    case PORT_CONNECT:
        port = (port_t*) HndGetPtr(NULL, params->port_connect.port, 'file');
        if (port == NULL)
        {
            *result = EINVALID;
            return false;
        ...
        if (!PortConnect(port, params->port_connect.remote))
        {
            *result = errno;
            return false;
        }
    ...
    case PORT_LISTEN:
        port = (port_t*) HndGetPtr(NULL, params->port_listen.port, 'file');
        if (port == NULL)
        {
            *result = EINVALID;
            return false;
    ...
    case PORT_ACCEPT:
        port = (port_t*) HndGetPtr(NULL, params->port_accept.port, 'file');
        if (port == NULL)
        {
            *result = EINVALID;
            return false;
        ...
        remote = PortAccept(port, params->port_accept.flags);
        if (remote)
            params->port_accept.client = HndDuplicate(NULL, &remote->hdr);
        else
        {
...
bool PortFsRequest(device_t* dev, request_t* req)
{
	...
	case FS_CREATE:
        ...
        wprintf(L"PortFsRequest(FS_CREATE): %s\n", req_fs->params.fs_create.name);
	...
	case FS_OPEN:
        ...
        wprintf(L"PortFsRequest(FS_OPEN): %s\n", req_fs->params.fs_open.name);
    ...
    case FS_PASSTHROUGH:
        return PortDoPortRequest(dev, req_fs->params.fs_passthrough.code,
            req_fs->params.fs_passthrough.params, &req_fs->header.result);
